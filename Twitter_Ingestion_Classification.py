...
#######################     PRE-PROCESSING  TWEETS    #######################################################################
        ...
        try:
            place_name = place_country = place_type = long = lat = '' #initalize to empty
            data = json.loads(data) #Parse the json response from API
            created_at = data['timestamp_ms']
            lang = data['lang']

            #Ignore if in any language beside English
            if data['lang'] != 'en':
                return
            
            #Ignore if its a retweet
            if 'retweeted_status' in data.keys():
                return
            
            ##These fields only available for geo-tagged tweets
            if data['place']:
...
#               return
            ...
            ##These fields only available if specifically sent with a tweet (Rare to have exact long/lat coordinates)
            if data['coordinates']:
                long = data['coordinates']['coordinates'][0]
                lat = data['coordinates']['coordinates'][1]
            
            ##If the tweet is truncated the attributes need to be pulled from the extended_tweet element
            if data['truncated']:
                tweet_text = unidecode(data['extended_tweet']['full_text'])
                hashtags = []
                for hashtag in data['extended_tweet']["entities"]["hashtags"]:    #iterate over the hastag list
                    hashtags.append(hashtag["text"])             #append each hashtag to 'hashtags'
            else:
                tweet_text = unidecode(data['text'])
                hashtags = []
                for hashtag in data["entities"]["hashtags"]:    #iterate over the hastag list
                    hashtags.append(hashtag["text"])             #append each hashtag to 'hashtags'
                
            hashtag_list = ','.join(map(str, hashtags)) 
...
#######################     LEMMATIZATION     ########################################################################## 
                ...
                cursor.execute("INSERT INTO tweets (created_at, tweet_text, keywords_strings, language, place_country,place_name,place_type,hashtags,long,lat, Naive_Bayes_Ind, Random_Forest_Ind, SVM_Ind, ensemble_Ind, Context_Entities) VALUES (?, ?, ?,?,?,?,?,?,?,?,?,?,?,?, ?)",
                    (created_at, tweet_text, keywords_strings, lang, place_country, place_name, place_type, hashtag_list, long, lat, Naive_Bayes_Ind, Random_Forest_Ind, SVM_Ind, ensemble_Ind, content_tweet))
